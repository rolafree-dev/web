...
export default function BracketPage() {
  ...
  const { data: tournaments, isLoading } = useCollection({ endpoint: '/tournaments' });
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      <div className="container mx-auto px-4 py-12 max-w-[1400px]">
        ...
        {!tournament ? (
          <div className="text-center py-32 bg-card/20 rounded-3xl border border-dashed border-muted">
            ...
            <h2 className="text-xl font-bold text-muted-foreground">No hay llaves activas en este momento</h2>
            <p className="text-sm text-muted-foreground/60 mt-1">Vuelve pronto para ver los próximos enfrentamientos.</p>
          </div>
        ) : (
        ...
        <div className="mt-16 flex flex-col md:flex-row items-center justify-between gap-8 p-10 bg-card border border-muted rounded-3xl">
          <div className="flex items-center gap-6">
            <div className="h-14 w-14 rounded-2xl bg-primary/10 flex items-center justify-center">
              <Star className="h-7 w-7 text-primary" />
            </div>
            <div>
              <h3 className="font-bold text-xl">Sistema de Competición Oficial</h3>
              <p className="text-muted-foreground text-sm max-w-sm">
                Los resultados se actualizan inmediatamente al finalizar cada fecha de la liga.
              </p>
            </div>
          </div>
          <Link href="/">
            <Button className="font-bold px-10 h-12 rounded-xl text-base shadow-lg shadow-primary/20">
              Ver Tabla General
            </Button>
          </Link>
        </div>
      ...
      <footer className="container mx-auto px-4 py-10 border-t border-muted/20 text-center mt-auto">
        <p className="text-[10px] font-bold text-muted-foreground/40 uppercase tracking-[0.5em]">
          Rola Free 2026 - Plataforma de Competición
        </p>
      </footer>
    </div>
  );
